...
fun MyIdentityScreen(
    ...
    val myFingerprint = remember(CryptoState.activeProfileUuid) { CryptoState.getMyFingerprint() }
    val myShortFingerprint = remember(CryptoState.activeProfileUuid) { CryptoState.getMyShortFingerprint() }
    val myBlob = remember(CryptoState.activeProfileUuid) { CryptoState.getMyExchangeBlob() }
    var recentlyRotated by remember { mutableStateOf(false) }

    LaunchedEffect(Unit) {
        recentlyRotated = withContext(Dispatchers.IO) {
            ProfileKeyManager.isKeyRecentlyRotated(context, CryptoState.activeProfileUuid)
        }
    }
    ...
    BiometricAuthTrigger(
        ...
        title = stringResource(R.string.identity_biometric_title),
        subtitle = stringResource(R.string.identity_biometric_subtitle),
        onSuccess = {
            ...
            Column(
                ...
                OutlinedCard(modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        ...
                        Spacer(modifier = Modifier.height(4.dp))
                                val chunkedFingerprint = remember(fingerprint) { fingerprint.chunked(4).joinToString(" ") }
                        Text(
                            text = chunkedFingerprint,
                            style = MaterialTheme.typography.bodyMedium
                        )
                    }
                }
